...
const LiveQuotesFeature: React.FC<LiveQuotesFeatureProps> = ({
  ...
  // NEW: call the hook
  const result = useVirtualList(stocks, {
  ...
  return (
    <>
      ...
      >
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            ...
            {spacerAbove > 0 && (
              <tr>
                <td colSpan={6} style={{ height: spacerAbove, padding: 0 }} />
              </tr>
            )}
            ...
            {spacerBelow > 0 && (
              <tr>
                <td colSpan={6} style={{ height: spacerBelow, padding: 0 }} />
              </tr>
            )}
          </tbody>
        </table>
      ...
      <p style={{ fontSize: 13, color: "#9CA3AF", marginTop: 6 }}>
        {stocks.length} total — {visibleItems.length} rows in the browser right
        now
      </p>
    </>
  );
